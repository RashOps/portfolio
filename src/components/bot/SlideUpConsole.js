...
export default function SlideUpConsole() {
	...
	const handleSubmit = (e) => {
		...
		try {
			track("atlas_prompt_sent", { promptLength: localInput.length });
		} catch (err) {}
						...
						<div className="flex-1 overflow-y-auto p-4 space-y-4">
							{messages.length === 0 && (
								<div className="text-on-surface-variant text-sm font-body space-y-2 p-4 glass rounded-xl">
									...
									<p>
										Je suis{" "}
										<span className="text-primary font-medium">{BOT_NAME}</span>
										, l'assistant IA de Rayhan. Posez-moi une question sur son
										profil, ses compétences ou ses projets.
									</p>
								</div>
							)}
									...
									if (toolParts.length > 0) {
										toolElements = toolParts.map((part) => (
											<div
												...
												className="text-primary/70 text-xs font-body mb-2 flex items-center gap-1"
											>
