	...
	const projects = [
		{
			...
			status: "Déployé",
			description:
				"Moteur RAG financier Production-Ready. Ingestion continue (RSS), vectorisation asynchrone (Cohere) et génération augmentée (Groq/Llama-3) via une API FastAPI robuste.",
		...
		},
		{
			...
			status: "Déployé",
			description:
				"Customer Sentiment & Insight Engine. Dashboard identifiant les frictions produits via l'analyse automatisée de feedbacks non structurés (Sentiment Analysis & Topic Modeling).",
		...
		},
		{
			...
			status: "Terminé",
			description:
				"Moteur de segmentation de marché en temps réel utilisant l'apprentissage non supervisé (Standardization → PCA → K-Means). API haute performance avec FastAPI et MongoDB.",
		...
		},
		{
			...
			status: "Terminé",
			description:
				"Système de surveillance temps réel garantissant l'intégrité de bases critiques via l'écoute d'événements SQL (Triggers) et la génération d'alertes par agent IA (OpenAI).",
		...
		},
		{
			...
			status: "Déployé",
			description:
				"Moteur de recommandation e-commerce basé sur le modèle RFM (Clustering K-Means) prescrivant des actions marketing ciblées (Rétention, Up-sell) selon les profils.",
		...
		},
		{
			...
			status: "Déployé",
			description:
				"Application web multi-pages de data visualisation complète. Harmonisation de datasets complexes et création de dashboards interactifs avec Plotly et Seaborn.",
		...
		},
		{
			...
			status: "Terminé",
			description:
				"Dashboard d'organisation personnelle (To-Do, Contacts, Notes) avec interface Glassmorphism. Architecture découplée React/Node.js et validation de données Joi.",
		...
		<div className="px-6 py-10 md:p-12 min-h-screen bg-background">
			...
			<section className="mb-12 mt-4" id="projects">
				...
				>
					...
					<span className="font-body text-[10px] md:text-sm text-on-surface-variant uppercase tracking-widest font-bold opacity-60">
				...
				>
					Une sélection de mes projets techniques alliant Data Science,
					Engineering et vision Business. Chaque mission illustre une
					transformation de données complexes en actifs stratégiques.
				</motion.p>
			</section>
						...
						>
							<div className="p-6 flex-grow flex flex-col">
								<div className="flex justify-between items-start mb-6">
									...
									<div
										className={`px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-tight ${project.status === "Déployé" ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-primary/10 text-primary border border-primary/20"}`}
									>
										{project.status}
									</div>
								</div>
										...
										<div className="flex gap-3 w-full translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
											{project.links.live && (
												<a
													...
													className="flex-1 bg-primary text-white text-center py-2.5 rounded-xl text-[10px] font-black uppercase hover:bg-primary-dim transition-all shadow-lg flex items-center justify-center gap-1.5 active:scale-95"
												>
											...
											<a
												...
												target="_blank"
												rel="noopener noreferrer"
												className={`flex-1 ${project.links.live ? "bg-white/10 backdrop-blur-md" : "bg-primary"} text-white text-center py-2.5 rounded-xl text-[10px] font-black uppercase hover:bg-white/20 transition-all shadow-lg flex items-center justify-center gap-1.5 active:scale-95`}
											>
												<span className="material-symbols-outlined text-sm">
													code
												</span>{" "}
								...
								<div className="flex flex-wrap gap-2 mt-auto pt-6 border-t border-white/5">
									{project.stack.map((tech) => (
										<span
											key={tech}
											className="px-3 py-1.5 bg-white/[0.03] border border-white/5 rounded-lg text-[9px] font-bold font-body text-on-surface-variant tracking-tighter hover:text-primary hover:border-primary/20 transition-colors"
										>
											{tech}
										</span>
									))}
			...
			<footer className="mt-24 mb-12 flex flex-col items-center">
				...
				>
					...
					<p className="font-body text-sm text-on-surface-variant font-medium">
						Explore more architecture on{" "}
						<a
							href="https://github.com/RashOps"
							target="_blank"
							rel="noopener noreferrer"
