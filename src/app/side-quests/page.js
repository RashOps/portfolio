	...
	const explorations = [
		{
			...
			status: "Terminé",
			description:
				"Dashboard d'organisation personnelle (To-Do, Contacts, Notes) avec une interface moderne en Glassmorphism. Architecture Fullstack découplée.",
		...
		},
		{
			...
			status: "Terminé",
			description:
				"Analyse exploratoire et harmonisation de datasets hétérogènes. Création d'une application multi-pages interactive pour l'aide à la décision.",
		...
		},
		{
			...
			status: "Terminé",
			description:
				"Système de monitoring de base de données PostgreSQL utilisant des Triggers et n8n pour envoyer des notifications intelligentes via OpenAI.",
			...
			<div className="px-6 py-10 md:p-12 relative overflow-hidden min-h-[calc(100vh-64px)]">
					...
					<div className="flex justify-between items-end gap-6 mb-8">
						<div>
							...
							<p className="text-on-surface-variant font-body text-sm max-w-2xl leading-relaxed">
								Projets personnels, expérimentations et sujets de curiosité.
								C'est ici que je teste de nouvelles idées en dehors du cadre
								académique.
							</p>
						</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
					{explorations.map((exp, i) => (
						<div
							key={i}
							className="glass rounded-2xl group card-hover flex flex-col relative overflow-hidden"
						>
							<div className="absolute top-4 right-4">
								<span className="text-xs font-body text-secondary bg-secondary/10 px-2.5 py-1 rounded-full font-medium">
									{exp.status}
								</span>
							</div>
							<div className="p-8">
								<div className="mb-6">
									<span
										className="material-symbols-outlined text-primary text-4xl mb-4 opacity-70 group-hover:opacity-100 transition-opacity"
								...
								<div className="space-y-4">
									<div>
										<div className="text-xs font-body text-on-surface-variant mb-3">
											Technologies :
										</div>
										<div className="flex flex-wrap gap-2">
											{exp.techs.map((tech) => (
												<span
													key={tech}
													className="glass rounded-lg px-3 py-1.5 text-[10px] font-body text-primary uppercase font-bold tracking-wider"
												>
													{tech}
												</span>
											))}
					...
					<div className="glass rounded-2xl flex flex-col relative opacity-30 grayscale pointer-events-none">
						<div className="absolute top-4 right-4">
							<span className="text-xs font-body text-on-surface-variant bg-white/5 px-2.5 py-1 rounded-full">
								Bientôt
							</span>
						</div>
						<div className="p-8">
							<div className="mb-6">
								...
								<h3 className="text-xl font-headline font-bold text-on-surface-variant">
									MLOps Pipeline
								</h3>
							</div>
							<p className="text-sm text-on-surface-variant/60 font-body mb-8">
								Mise en production de modèles ML avec monitoring et versioning
								automatisé.
							</p>
