...
export default function MarketPulseDemo() {
	...
	const mockSegments = [
		{
			business_segment: "Growth Tech",
			color: "text-primary",
			asset_count: 124,
			feature_volatility_pct: "4.2%",
			feature_momentum_pct: "+2.1%",
		},
		{
			business_segment: "High Volatility",
			color: "text-error",
			asset_count: 45,
			feature_volatility_pct: "12.5%",
			feature_momentum_pct: "-1.5%",
		},
		{
			business_segment: "Defensive Value",
			color: "text-secondary",
			asset_count: 89,
			feature_volatility_pct: "1.2%",
			feature_momentum_pct: "+0.4%",
		},
	];
	...
	const fetchEngineData = useCallback(
		async (showLoading = false) => {
			...
			try {
				const [_segResult, _realNews, _realMetrics, _inventory, _config] =
					await Promise.all([
				...
				if (_inventory) setTotalAssets(_inventory.total_raw_records);
					...
					setTotalAssets(_segResult.metadata?.total_assets || 0);

				if (_config) setEngineConfig(_config);
				if (Array.isArray(_realNews) && _realNews.length > 0)
					setNews(_realNews);
			...
			<div className="relative max-w-[1600px] mx-auto px-4 py-6 md:px-8 md:py-10 grid grid-cols-1 xl:grid-cols-[280px_1fr_320px] gap-8 h-full min-h-[90vh]">
				...
				<aside className="space-y-8 flex flex-col order-2 xl:order-1">
					...
					<section className="space-y-4">
						<Link
							href="/decrypt-demos"
							className="inline-flex items-center gap-2 text-[10px] uppercase font-bold text-on-surface-variant hover:text-primary transition-colors mb-2"
						>
						...
						<div className="space-y-1">
							...
							<div className="flex items-center gap-2">
								<span className="text-[10px] font-mono text-primary bg-primary/10 px-2 py-0.5 rounded-full uppercase tracking-tighter border border-primary/20">
									CORE V1.2.0
								</span>
								<div
									className={`w-1.5 h-1.5 rounded-full ${apiStatus === "online" ? "bg-green-400" : "bg-orange-400"} animate-pulse`}
								/>
							</div>
						</div>
						<p className="text-[11px] leading-relaxed text-on-surface-variant/80 uppercase tracking-widest">
							{systemMessage}
						</p>
					</section>
					...
					<div className="glass rounded-2xl p-5 border border-white/5 space-y-4">
						<div className="flex justify-between items-end border-b border-white/5 pb-4">
							<div className="space-y-1">
								<span className="text-[10px] text-on-surface-variant uppercase font-bold opacity-60">
									Asset Inventory
								</span>
								<div className="font-headline text-2xl font-bold gradient-text">
									{totalAssets || "---"}
								</div>
							</div>
							<button
						...
						<div className="grid grid-cols-1 gap-3 pt-2">
							<div className="flex justify-between text-[10px] font-bold">
								...
								<span className="text-secondary">
									{engineConfig?.pca_components || 2} AXIS
								</span>
							</div>
							<div className="flex justify-between text-[10px] font-bold">
								<span className="opacity-40 uppercase tracking-widest">
								...
								<span className="text-tertiary">
									{engineConfig?.max_clusters || 8} RANGE
								</span>
							</div>
						</div>
					</div>
					...
					<div className="space-y-4">
						<h3 className="text-[10px] uppercase font-bold tracking-[0.3em] opacity-40 px-2">
							Command Center
						</h3>
							...
							{[
								{
									...
									icon: "query_stats",
									color: "text-secondary",
									action: triggerTickerScraping,
									active: tickerScraping,
									sub: "yFinance / S&P500",
								},
								{
									label: "Scrape Intelligence",
									icon: "rss_feed",
									color: "text-primary",
									action: triggerNewsScraping,
									active: scraping,
									sub: "Market Feeds Ingestion",
								},
								{
									label: "Optimize Model",
									icon: "model_training",
									color: "text-tertiary",
									action: triggerTraining,
									active: retraining,
									sub: "Latent Space Projection",
								},
							].map((btn, i) => (
								<button
									key={i}
								...
								>
									<div className="flex items-center gap-4 relative z-10">
										<div
											className={`w-10 h-10 rounded-lg bg-surface-container flex items-center justify-center border border-white/5 group-hover:scale-110 transition-transform ${btn.color}`}
										>
											<span
												className={`material-symbols-outlined ${btn.active ? "animate-spin" : ""}`}
											>
												{btn.icon}
											</span>
										</div>
				...
				<main className="order-1 xl:order-2 space-y-8">
					<div className="glass rounded-[24px] md:rounded-[32px] p-5 md:p-8 min-h-[400px] md:min-h-[600px] flex flex-col border border-white/10 relative overflow-hidden group">
						...
						<div className="flex justify-between items-start relative z-10 mb-8">
							<div>
								<h2 className="font-headline text-xl md:text-2xl font-bold mb-1">
									Topological Market Mapping
								</h2>
								<div className="flex items-center gap-4">
									<p className="text-[10px] text-on-surface-variant uppercase font-bold tracking-widest opacity-60">
										Mapping de {formattedPca.length} regimes latents
									</p>
									<div className="h-[1px] w-8 bg-white/10" />
									<div className="flex gap-4">
										<div className="flex items-center gap-2">
											<div className="w-2 h-2 rounded-full bg-primary shadow-[0_0_8px_#a78bfa]" />
											<span className="text-[8px] uppercase font-bold opacity-40">
												Main Stream
											</span>
										</div>
										<div className="flex items-center gap-2">
							...
							<div className="text-right">
								<div className="text-[10px] font-bold opacity-30 uppercase mb-1">
									Silhouette Accuracy
								</div>
						...
						<div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 pt-8 border-t border-white/5 relative z-10">
							{[
								{
									label: "Explained Var.",
									value: metrics?.results?.total_variance_explained
										? (metrics.results.total_variance_explained * 100).toFixed(
												1,
											) + "%"
										: "---",
									color: "text-primary",
								...
								{
									label: "Cluster Regimes",
									value: metrics?.results?.optimal_k || "---",
									color: "text-tertiary",
								},
								{
								...
								<div key={i} className="space-y-1">
									<div className="text-[9px] uppercase font-bold opacity-30 tracking-widest">
										{m.label}
									</div>
									<div className={`font-headline text-lg font-bold ${m.color}`}>
										{m.value}
									</div>
								</div>
							))}
						...
						{segments.map((segment, i) => (
							...
							>
								...
								<div className="flex justify-between items-start mb-6">
									<div>
										<h3
											className={`font-headline font-bold text-base tracking-tight mb-1 ${segment.color || "text-primary"}`}
										>
											{segment.business_segment}
										</h3>
										<div className="flex items-center gap-2">
											<span className="text-[10px] font-bold text-on-surface-variant bg-white/5 px-2 py-0.5 rounded uppercase">
												{segment.asset_count} assets
											</span>
										</div>
									</div>
								</div>
								...
								<div className="grid grid-cols-2 gap-6 pt-4 border-t border-white/5">
									<div className="space-y-1">
										<span className="text-[9px] uppercase text-on-surface-variant font-bold opacity-40 tracking-wider">
											Volatility Median
										</span>
										<div className="text-sm font-bold text-white">
											{segment.feature_volatility_pct || "---"}
										</div>
									</div>
									<div className="space-y-1">
										<span className="text-[9px] uppercase text-on-surface-variant font-bold opacity-40 tracking-wider">
										...
										<div
											className={`text-sm font-bold ${parseFloat(segment.feature_momentum_pct) >= 0 ? "text-green-400" : "text-red-400"}`}
										>
							...
							<div className="flex gap-1">
								{["All", "Yahoo Finance", "Investing.com"].map((src) => (
									<button
										key={src}
										onClick={() => {
											setNewsSource(src === "All" ? null : src);
											setNewsPage(0);
										}}
