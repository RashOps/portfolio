...
export default function FinSightDashboard() {
	...
	const fetchArticlesList = async () => {
		...
		const result = await finSightApiService.getArticles(
			0,
			20,
			null,
			articleFilter,
		...
		if (result) {
			...
			setChatHistory((prev) => [
				...prev,
				{
					...
					role: "system",
					content:
						"Erreur de connexion à l'API RAG. Le service est peut-être en veille sur Hugging Face.",
					timestamp: new Date().toISOString(),
				},
			]);
		}
	...
	const handleTriggerScraping = async () => {
		...
		setTimeout(() => {
			setIngestionMessage(
				result
					? "Scraping démarré avec succès. Actualisez dans quelques instants."
					: "Erreur lors du lancement du scraper.",
			);
			setIsScraping(false);
	...
	const handleTriggerVectorization = async () => {
		...
		setTimeout(() => {
			setIngestionMessage(
				result
					? "Vectorisation asynchrone démarrée."
	...
	return (
		<div className="min-h-screen bg-background text-on-surface font-body p-4 md:p-8 pt-24 md:pt-28">
			...
			<div className="max-w-7xl mx-auto mb-8">
				<Link
					href="/decrypt-demos"
					className="inline-flex items-center gap-2 text-green-400 hover:text-green-300 transition-colors text-xs font-bold uppercase tracking-widest mb-6"
				>
				...
				<div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-white/10 pb-6">
					<div>
						<div className="flex items-center gap-3 mb-2">
							...
							<h1 className="text-3xl md:text-5xl font-headline font-black tracking-tight">
								FinSight <span className="text-green-400">RAG</span>
							</h1>
						</div>
						<p className="text-on-surface-variant max-w-2xl text-sm leading-relaxed">
							Moteur RAG (Retrieval-Augmented Generation) financier. Interrogez
							l'actualité des marchés, le moteur extraira les documents
							vectorisés de Qdrant pour générer une synthèse via Groq (Llama-3).
						</p>
					</div>

					<div className="flex items-center gap-4 glass px-5 py-3 rounded-xl border border-green-400/20 bg-green-400/5">
						<div className="flex flex-col">
							<span className="text-[10px] text-green-400/70 uppercase font-bold tracking-widest mb-1">
								FastAPI Backend
							</span>
							<div className="flex items-center gap-2">
								<span
									className={`w-2 h-2 rounded-full ${health?.status === "healthy" ? "bg-green-400 animate-pulse" : "bg-red-400"}`}
								></span>
			...
			<div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
				...
				<div className="lg:col-span-7 flex flex-col gap-8">
					...
					<motion.div
						...
						className="glass rounded-2xl border border-white/10 flex flex-col h-[600px] overflow-hidden"
					>
						...
						<div className="flex-grow flex flex-col relative bg-surface-container-low/30 min-h-0">
							...
							<div className="flex-grow p-5 overflow-y-auto custom-scrollbar flex flex-col gap-6 min-h-0">
								{chatHistory.map((msg) => (
									...
									>
										<div
											className={`max-w-[85%] rounded-2xl p-4 ${msg.role === "user" ? "bg-green-400 text-background rounded-tr-sm" : msg.role === "system" ? "bg-red-400/10 border border-red-400/30 text-red-400 rounded-tl-sm" : "bg-surface-container border border-white/5 rounded-tl-sm"}`}
										>
											...
											{/* Message Content */}
											<div
												className={`text-sm leading-relaxed ${msg.role === "user" ? "font-medium whitespace-pre-wrap" : "text-on-surface prose prose-invert prose-sm max-w-none prose-p:leading-relaxed prose-pre:bg-black/50 prose-pre:border prose-pre:border-white/10 prose-headings:font-headline prose-a:text-green-400"}`}
											>
							...
							<form
								...
								className="p-4 border-t border-white/10 bg-surface-container-low flex flex-col shrink-0 relative z-10"
							>
					...
					>
						...
						<div className="glass rounded-xl p-5 border border-white/5 flex flex-col justify-between">
							...
							>
								{isScraping ? (
									<>
										<span className="w-3 h-3 border-2 border-green-400/20 border-t-green-400 rounded-full animate-spin"></span>{" "}
										Scraping...
									</>
								) : (
									<>
									...
									<ul className="space-y-3 font-mono text-xs text-on-surface-variant mb-6">
										<li className="flex justify-between items-center">
											...
											<span className="text-purple-400 font-bold">
												{articlesStatus.vectorized_articles}
											</span>
										</li>
										<li className="flex justify-between items-center">
											<span>Pending Queue</span>
											<span className="text-orange-400 font-bold">
												{articlesStatus.non_vectorized_articles}
											</span>
										</li>
									</ul>
								) : (
				...
				<motion.div
					...
					className="lg:col-span-5 glass rounded-2xl border border-white/10 flex flex-col h-[700px] lg:h-[840px]"
				>
									...
									{articles.map((art, idx) => (
										...
										>
											...
											<div className="flex justify-between items-end mt-auto pt-3 border-t border-white/5">
												...
												<div
													className={`px-2 py-1 rounded text-[9px] font-bold uppercase tracking-wider flex items-center gap-1 ${art.vectorized ? "bg-purple-400/10 text-purple-400" : "bg-orange-400/10 text-orange-400"}`}
												>
													<span className="material-symbols-outlined text-[10px]">
														{art.vectorized ? "check_circle" : "pending"}
													</span>
